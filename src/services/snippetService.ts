...
export class SnippetService {
  ...
  static async createSnippet(snippetData: {
    ...
    try {
      ...
      const snippet: Snippet = {
        ...
        code: result.code,
        createdAt: new Date(
          "localId" in result ? result.createdAt : result.createdAt
        ),
        updatedAt: new Date(
          "localId" in result ? result.updatedAt : result.updatedAt
        ),
      };
  ...
  static async loadSnippets(): Promise<{ error: string | null }> {
    ...
    try {
      ...
      if (navigator.onLine) {
        try {
          const { data: serverSnippets, error } =
  ...
  private static mergeSnippets(
    ...
    for (const localSnippet of localSnippets) {
      const existsOnServer = serverSnippets.some(
        (serverSnippet) =>
          serverSnippet.title === localSnippet.title &&
          serverSnippet.language === localSnippet.language &&
          Math.abs(
            new Date(serverSnippet.createdAt).getTime() -
              new Date(localSnippet.createdAt).getTime()
