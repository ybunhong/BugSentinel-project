import { LocalStorageService, type LocalSnippet, type LocalPreferences } from './localStorageService';
...
export class SyncService {
  ...
  private static async syncFromServer(): Promise<void> {
    ...
    try {
      ...
      const { data: serverSnippets, error: snippetsError } = await SupabaseService.getSnippets(user.id);
      ...
      const { data: serverPreferences, error: prefsError } = await SupabaseService.getUserPreferences(user.id);
  ...
  private static async syncCreateSnippet(localSnippet: LocalSnippet, userId: string): Promise<void> {
    const snippetData = {
      userId,
      title: localSnippet.title,
      language: localSnippet.language,
      code: localSnippet.code,
      analysisResults: localSnippet.analysisResults
    };
  ...
  private static async syncUpdateSnippet(localSnippet: LocalSnippet, userId: string): Promise<void> {
    ...
    const matchingSnippet = serverSnippets?.find(s => 
      s.title === localSnippet.title && s.language === localSnippet.language
    );

    if (matchingSnippet) {
      const { error } = await SupabaseService.updateSnippet(matchingSnippet.id, {
        title: localSnippet.title,
        language: localSnippet.language,
        code: localSnippet.code,
        analysisResults: localSnippet.analysisResults
  ...
  private static async syncDeleteSnippet(localSnippet: LocalSnippet, userId: string): Promise<void> {
  ...
  private static async syncPreferences(localPreferences: LocalPreferences, userId: string): Promise<void> {
  ...
  static async updateSnippetOfflineFirst(
    ...
    if (this.isOnline && user) {
      // Try online first
      try {
        const { data, error } = await SupabaseService.updateSnippet(snippetId, updates);
        if (!error && data) {
          return data;
        }
      } catch (error) {
    ...
    // Fallback to local storage
    return LocalStorageService.updateLocalSnippet(snippetId, updates as Partial<LocalSnippet>);
  }
