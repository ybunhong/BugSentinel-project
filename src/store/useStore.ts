...
interface StoreActions {
  ...
  setRefactorResult: (result: RefactorResult | null) => void;
  setCodeSuggestions: (suggestions: CodeSuggestion[]) => void;
    ...
    (set) => ({
      ...
      deleteSnippet: (id) =>
        set((state) => ({
          ...
          currentSnippet:
            state.currentSnippet?.id === id ? null : state.currentSnippet,
        })),
      ...
      setRefactorResult: (refactorResult) => set({ refactorResult }),
      setCodeSuggestions: (codeSuggestions) => set({ codeSuggestions }),

      // UI actions
      setTheme: (theme) => {
        set({ theme });
        // Sync theme preference to Supabase (async, don't wait)
        import("../services/preferencesService").then(
          ({ PreferencesService }) => {
